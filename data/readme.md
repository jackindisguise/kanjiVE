...
Details all of the elements that are used and what kanji they are used in.
Identical to `elements.json`, just in text format.

## `jouyou.json`
* Uses data from `jouyou.txt`
* Generated by `prepare.js`

All of the data from `jouyou.txt` in a more usable format.
...
A text representation of the **KanjiVG** dataset.

## `keywords.json`
* Uses data from `keywords.txt`
* Generated by `prepare.js`

All of the data from `keywords.txt` in a usable format.
...
Allows use to use **KanjiVG** "type" fields to associate more complex elements that **KanjiVG** diegned not to include in its dataset.
